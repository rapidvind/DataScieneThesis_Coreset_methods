...
# Flatten loader for herding
flat_loader = DataLoader(full_train_dataset, batch_size=batch_size, shuffle=False)
X = []
for x, _ in flat_loader:
    X.append(x)
X = torch.cat(X, dim=0).view(len(full_train_dataset), -1)  # shape: [50000, 3072]

def herding(X, subset_size):
    mean = torch.mean(X, dim=0)
    selected = []
    summed = torch.zeros_like(mean)
    mask = torch.zeros(len(X), dtype=torch.bool)

    for _ in tqdm(range(subset_size)):
        residual = mean - summed / (len(selected) + 1e-9)
        scores = torch.matmul(X, residual)
        scores[mask] = -float('inf')
        selected_idx = torch.argmax(scores).item()
        selected.append(selected_idx)
        summed += X[selected_idx]
...
print("Running Herding selection... This may take a few minutes.")
start_time = time.time()
selected_indices = herding(X, subset_size)
